...
const ExportQuerySchema = z.object({
  ...
  aggregateMetrics: z.string().optional().transform(val => val === undefined ? undefined : val === 'true'),
  ...
  startDate: z.string().transform((val) => {
    ...
    // Convert date-only to datetime at start of day (00:00:00)
  ...
  endDate: z.string().transform((val) => {
    if (!val) return undefined;
    // Accept both date (YYYY-MM-DD) and datetime formats
    // Convert date-only to datetime at end of day (23:59:59)
    if (/^\d{4}-\d{2}-\d{2}$/.test(val)) {
...
router.get('/csv', validateExportQuery, ErrorUtils.catchAsync(async (req: Request, res: Response): Promise<void> => {
  const query = (req as any).validatedQuery as ExportQuery;
  ...
  // Fetch all matching campaigns by paginating through results
  ...
  while (hasMore) {
    const campaignQuery: DataWarehouseCampaignQuery = {
      ...
      ...(query.isServing !== undefined && { isServing: query.isServing }),
    ...
    if (campaignsResult.data && campaignsResult.data.length > 0) {
      allCampaigns = allCampaigns.concat(campaignsResult.data);
      hasMore = campaignsResult.data.length === 100; // Continue if we got a full page
      page++;
    } else {
      hasMore = false;
    }
  ...
  logger.info('Campaign query for export completed', { totalCampaigns: campaigns.length });
...
router.get('/json', validateExportQuery, ErrorUtils.catchAsync(async (req: Request, res: Response): Promise<void> => {
  ...
  while (hasMore) {
    ...
    if (campaignsResult.data && campaignsResult.data.length > 0) {
      allCampaigns = allCampaigns.concat(campaignsResult.data);
      hasMore = campaignsResult.data.length === 100;
      page++;
    } else {
      hasMore = false;
    }
